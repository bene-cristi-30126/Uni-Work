...
{ /* n - numărul nodurilor, c - matricea costurilor, muchii - muchiile arborelui de acoperire de cost minim, cost - costul arborelui de acoperire de cost minim */
     int i, j, k, minim , v[NMAX];
    /* v[i] = 0 dacă i aparține arborelui, v[i] = j dacă i nu aparține arborelui, j este nodul din arbore a.î. muchia (i, j) este de cost minim*/
     *cost = 0;
     v[1] = 0;
     /* arborele este ({1},{}) */
     for(i = 2; i <= n; i++)
     ...
     { /* determinarea muchiei care se adaugă arborelui */
         ...
         muchii[i][0] = v[j];
         muchii[i][1] = j;
         *cost = *cost + c[j][v[j]]; /* reactualizare vector v */
         v[j] = 0;
         for(k = 1; k <= n; k++)
