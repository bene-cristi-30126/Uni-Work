...
{ /* s = nod de plecare */
    ...
    for (i = 1; i <= n; i++)
    {
        vizitate[i] = 0;
        distante[i] = cost[s][i];
        if (distante[i] < INFINIT) tata[i] = s; //lega nodurile de start
        else tata[i] = 0;
    }
    vizitate[s] = 1;
    tata [s] = 0; //tata nodului de start
    distante [s] = 0;
    for (pas = 1; pas <= n-1; pas++)
    {
        /* găsește vârful k neselectat cu distante[k] minim */
        minim = INFINIT;
        k = 0;
        for (i = 1; i <= n; i++)
        {
            ...
            {
                minim = distante[i];//obtine o ruta
                k = i;
            }
        }
        if (minim == INFINIT)
