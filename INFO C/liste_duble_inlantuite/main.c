...
DATE_UTILE *Aloca_date(char *cuvant); /* alocă memorie pentru un cuvânt, inițial frcvențaeste 1 */
NOD *Aloca_nod(DATE_UTILE *date_utile); /* alocă memorie pentru un nod al listei dubluînlănțuite */
LISTA *Aloca_lista(); /* alocă memorie pentru structura LISTĂ, inițializează prim, ultim,dimensiune cu 0 */
void Cauta(LISTA *l, char *cuvant); /* l->crt va conține pointer către nodul ce are pointerla date utile cuvântul căutat sau NULL dacă nu există cuvântul căutat în listă */
void Adauga_ultim(LISTA *l, NOD *nod); /* adaugă nodul nod după ultimul nod din lista l */
void Insereaza_prim(LISTA *l, NOD *nod); /* inserează nodul nod înainte de primul nod dinlista l */
void Insereaza(LISTA *l, NOD *nod); /* inserează nodul nod înainte de nodul curent din listal (înainte de l->crt) */

int main()
{
    LISTA *l1 = Aloca_lista(), *l2 = Aloca_lista();
    Creare(l1, l2);
    Afisare(l1, 0, "Cuvintele in ordinea citirii");
    Afisare(l2, 0, "Cuvintele distincte in ordinea crescatoare");
    Afisare(l2, 1, "Cuvintele distincte in ordinea descrescatoare");
    return 0;
}
    ...
    for(i=1; i<=n; i++)
        {
            ...
            { /* cuvântul citit există deja, nu mai trebuie alocată memorie, se creștefrecvența și se adaugă în l1 după ultim */
